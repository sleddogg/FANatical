...
import { defaultHomeCustomization, useHomeCustomization } from "../data/homeCustomizationPreference";
...
export function HomePage() {
  ...
  const visibleCustomization = configured || prototypeMode ? customization : defaultHomeCustomization;
  const visibleSide = configured || prototypeMode ? side : "left";
  const visibleImages = configured || prototypeMode ? images : {};
  const frameRef = useRef<HTMLElement>(null);
  const navigationRef = useRef<HTMLElement>(null);
  const textOverlayRef = useRef<HTMLDivElement>(null);
  const fanCardRef = useRef<HTMLDivElement>(null);
  const [storedProfile, setStoredProfile] = useState<ProfileRecord | null>(() => prototypeMode ? initialProfile : null);
  ...
  const preferredPositions = useMemo<ResolvedHomeOverlayPositions>(() => ({
    ...(visibleCustomization.textOverlay.enabled ? { textOverlay: visibleCustomization.textOverlay.position } : {}),
    ...(visibleCustomization.fanCard.enabled ? { fanCard: visibleCustomization.fanCard.position } : {}),
  ...
  useEffect(() => {
    const clear = () => {
      setStoredProfile(prototypeMode ? initialProfile : null);
  ...
  const activeImage = visualLayout === "wide" ? visibleImages.wide ?? visibleImages.mobile : visibleImages.mobile ?? visibleImages.wide;
  const showTextOverlay = visibleCustomization.textOverlay.enabled;
  const showFanCard = visibleCustomization.fanCard.enabled && fanCardFields.length > 0;
  ...
  return (
    <section className="home-hero" aria-label="FANatical Home" data-profile-visual-layout={visualLayout} data-has-custom-image={Boolean(activeImage)} ref={frameRef}>
      ...
      <nav ref={navigationRef} className={`home-hero__shortcuts home-hero__shortcuts--${visibleSide}`} aria-label="Feature shortcuts">
      ...
      {showTextOverlay ? <div ref={textOverlayRef} className="home-hero__overlay home-hero__text-overlay" data-position={resolvedPositions.textOverlay ?? visibleCustomization.textOverlay.position} data-requested-position={visibleCustomization.textOverlay.position}>
        {visibleCustomization.textOverlay.bigText ? <h1 id="home-title">{visibleCustomization.textOverlay.bigText}</h1> : null}
        {visibleCustomization.textOverlay.smallText ? <p>{visibleCustomization.textOverlay.smallText}</p> : null}
      </div> : null}

      {showFanCard ? <div ref={fanCardRef} className={`home-hero__overlay home-hero__fan-card home-hero__fan-card--${visibleCustomization.fanCard.layout}`} data-position={resolvedPositions.fanCard ?? visibleCustomization.fanCard.position} data-requested-position={visibleCustomization.fanCard.position} aria-label="Fan Card">
