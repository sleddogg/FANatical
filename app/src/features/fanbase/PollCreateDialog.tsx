  ...
  return (
    <div className="fanbase-dialog-layer poll-dialog-layer" role="presentation">
      ...
      <section className="poll-dialog poll-create-dialog" role="dialog" aria-modal="true" aria-labelledby="create-poll-title">
        <header><div><span className="eyebrow">FANbase Polls</span><small>One Poll, one canonical scope</small></div><h2 id="create-poll-title">Create Poll</h2><button type="button" aria-label="Cancel Create Poll" onClick={() => onClose()}>×</button></header>
        <div className="poll-create-dialog__body">
          ...
          <fieldset className="poll-create-options"><legend>Answer options</legend>{options.map((option, index) => <div className="poll-create-option-row" key={index}><label><span>Option {index + 1}</span><input value={option} onChange={(event) => setOptions((current) => current.map((value, optionIndex) => optionIndex === index ? event.target.value : value))} /></label>{options.length > 2 ? <button type="button" aria-label={`Remove option ${index + 1}`} onClick={() => setOptions((current) => current.filter((_, optionIndex) => optionIndex !== index))}>×</button> : null}</div>)}{options.length < 6 ? <button className="poll-create-add-option" type="button" onClick={() => setOptions((current) => [...current, ""])}>+ Add option</button> : null}</fieldset>
