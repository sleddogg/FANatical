...
export const mockFanConnections: readonly CommunityUser[] = Object.values(users);
...
export const initialFanPhotos: readonly FanPhoto[] = [
  ...
  fanPhoto({ id: "photo-pats-game-face-pair", teamId: "new-england-patriots", owner: users.road, category: "Game Face", title: "Double coverage", details: "Two longtime seat neighbors, one coordinated game-day plan.", images: [fanPhotoImages.gameFaceEaglesPair], hoursAgo: 54, ratingTotal: 190, ratingCount: 52, reactions: { Like: 24, Love: 20, Fire: 14 }, rankingBadge: "Top 100" }),
...
export const initialEvents: readonly FanEvent[] = [
  { id: "event-pats-watch", teamId: "new-england-patriots", title: "Opening Night Watch Party", eventType: "Watch Party", startsAt: relativeDate(72), location: { label: "Harbor Street Social · Boston" }, host: "Boston North Fans", visibility: "Public", description: "Big screens, reserved FANatical section, and a pregame prediction board.", invitedUserIds: [users.maya.id, users.road.id], joinCount: 84, joined: false, saved: false, reported: false },
  { id: "event-pats-meetup", teamId: "new-england-patriots", title: "Road Crew Planning Meetup", eventType: "Meetup", startsAt: relativeDate(124), location: { label: "Online" }, host: "New England Road Crew", visibility: "Private", description: "Coordinate travel, tickets, and the group tailgate for the next away game.", invitedUserIds: [users.road.id], joinCount: 26, joined: true, saved: true, reported: false },
  { id: "event-sox-rivalry", teamId: "boston-red-sox", title: "Rivalry Series Patio Night", eventType: "Rivalry Event", startsAt: relativeDate(48), location: { label: "Back Bay Baseball Club" }, host: "Fenway Faithful", visibility: "Public", description: "A three-inning trivia sprint before first pitch and a shared game watch.", invitedUserIds: [], joinCount: 119, joined: false, saved: false, reported: false },
  { id: "event-celtics-online", teamId: "boston-celtics", title: "Rotation Roundtable", eventType: "Online Event", startsAt: relativeDate(36), location: { label: "Online" }, host: "Green Line Supporters", visibility: "Public", description: "A casual video meetup to compare lineup ideas before the season starts.", invitedUserIds: [users.green.id], joinCount: 63, joined: false, saved: true, reported: false },
];

export const initialGroups: readonly FanGroup[] = [
  { id: "group-pats-road-crew", teamId: "new-england-patriots", name: "New England Road Crew", description: "Away-game travel plans, ticket tips, and meetup coordination.", visibility: "Public", memberCount: 318, joined: true, viewerRole: "Owner", memberUserIds: [demoUser.id, users.road.id, users.maya.id, users.coach.id], invitedUserIds: [], moderatorUserIds: [users.road.id], latestActivity: relativeDate(-1.4), threadId: "group-pats-road-crew-thread", reported: false },
  { id: "group-pats-family", teamId: "new-england-patriots", name: "Sunday Family Huddle", description: "A small private group for family picks and game-day photos.", visibility: "Private", memberCount: 9, joined: true, viewerRole: "Moderator", memberUserIds: [demoUser.id, users.maya.id, users.road.id], invitedUserIds: [], moderatorUserIds: [demoUser.id], latestActivity: relativeDate(-5), threadId: "group-pats-family-thread", reported: false },
  { id: "group-pats-film", teamId: "new-england-patriots", name: "Foxborough Film Club", description: "Public weekly breakdowns without the hot-take noise.", visibility: "Public", memberCount: 542, joined: false, viewerRole: null, memberUserIds: [users.coach.id, users.maya.id], invitedUserIds: [], moderatorUserIds: [users.coach.id], latestActivity: relativeDate(-0.8), threadId: "group-pats-film-thread", reported: false },
  { id: "group-sox-local", teamId: "boston-red-sox", name: "Fenway Faithful", description: "Local game talk and last-minute meetup planning.", visibility: "Public", memberCount: 741, joined: false, viewerRole: null, memberUserIds: [users.fenway.id], invitedUserIds: [], moderatorUserIds: [users.fenway.id], latestActivity: relativeDate(-0.6), threadId: "group-sox-local-thread", reported: false },
  { id: "group-celtics-green", teamId: "boston-celtics", name: "Green Line Supporters", description: "City-wide Celtics chat, events, and road-game watches.", visibility: "Public", memberCount: 886, joined: true, viewerRole: "Member", memberUserIds: [demoUser.id, users.green.id], invitedUserIds: [], moderatorUserIds: [users.green.id], latestActivity: relativeDate(-0.4), threadId: "group-celtics-green-thread", reported: false },
];
