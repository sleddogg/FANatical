  ...
  return (
    <div className="internal-modal" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="exception-editor-title" className="internal-dialog internal-dialog--wide">
        <header><div><span className="internal-kicker">Boundary routing</span><h2 id="exception-editor-title">Section {section.section} exceptions</h2><p>Use a row range, seat range, or both. The first matching override for each axis wins.</p></div><button type="button" aria-label="Close exceptions" onClick={onClose}><AppIcon name="x-mark" /></button></header>
        ...
        <footer><span>Changes save immediately.</span><button type="button" onClick={add}><AppIcon name="plus" /> Add Exception Rule</button><button className="internal-primary" type="button" onClick={onClose}>Done</button></footer>
      </section>
    </div>
  );
}
    ...
    <InternalVenueShell mode="mapping">
      <main className="internal-venue-main">
        <section className="venue-hero">
          <div><span className="internal-kicker">Seeded working venue</span><h1>{venue.name}</h1><p>{venue.location}</p><div className="venue-hero__actions"><Link className="internal-primary" to="/internal/venues/rexall-place/test">Open Seat Resolver Tester <AppIcon name="arrow-right" /></Link><span>Saved locally · Updated {new Date(venue.updatedAt).toLocaleString()}</span></div></div>
        ...
        <section className="internal-panel"><header className="internal-section-heading"><div><span className="internal-kicker">Event context, not physical mapping</span><h2>Team Seating Profiles</h2><p>Profiles describe the portions of Rexall Place a team normally uses. Resolver output remains physical.</p></div><button className="internal-primary" type="button" onClick={() => setProfileDraft({ id: newId("team-profile"), teamName: "", levels: "Upper + Lower", sides: "Both", ends: "Both" })}><AppIcon name="plus" /> Add Team Profile</button></header><div className="team-profile-grid">{venue.teamProfiles.map((profile) => <article key={profile.id}><div><span className="team-profile-mark" aria-hidden="true">{profile.teamName.split(" ").map((word) => word[0]).join("").slice(0, 2)}</span><div><h3>{profile.teamName}</h3><p>{profile.levels} · {profile.sides === "Both" ? "Both sides" : profile.sides} · {profile.ends === "Both" ? "Both ends" : profile.ends}</p></div></div><div><button type="button" onClick={() => setProfileDraft(profile)}>Edit</button><button type="button" onClick={() => { if (window.confirm(`Delete ${profile.teamName}?`)) setVenue((current) => ({ ...current, teamProfiles: current.teamProfiles.filter((item) => item.id !== profile.id) })); }}>Delete</button></div></article>)}</div></section>
      </main>
