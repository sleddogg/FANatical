    ...
    <div className="cheer-dialog-layer">
      ...
      <section className="cheer-check-in" role="dialog" aria-modal="true" aria-labelledby="cheer-check-in-title">
        ...
        {stage === "mappedDetails" ? <form className="cheer-check-in__details" onSubmit={reviewMapped}>
          ...
          <label>Sport<select required disabled={!selectedVenue} value={sports.includes(draft.sport) ? draft.sport : ""} onChange={(event) => setDraft((current) => ({ ...current, sport: event.target.value as CheerSport, teamEvent: "" }))}><option value="" disabled>Choose sport</option>{sports.map((sport) => <option key={sport}>{sport}</option>)}</select></label>
          {teamEvents.length ? <label>Team / Event<select required value={draft.teamEvent} onChange={(event) => setDraft((current) => ({ ...current, teamEvent: event.target.value }))}><option value="" disabled>Choose Team / Event</option>{teamEvents.map((team) => <option key={team.id} value={team.teamName}>{team.teamName}</option>)}</select></label> : <label>Team / Event <small>Optional</small><input value={draft.teamEvent} placeholder="Team, matchup, or event" onChange={(event) => setDraft((current) => ({ ...current, teamEvent: event.target.value }))} /></label>}
